...
import { trophy, flame, book, ribbon, star } from 'ionicons/icons';
import { ProgressBar } from '../components/ui/ProgressBar';
import '../styles/Profile.css';
...
const Profile: React.FC = () => {
  ...
  const mockAchievements = [
    ...
    {
      id: "3",
      title: "Maître du Ndolé",
      description: "Devenir expert en cuisine camerounaise",
      unlocked: false
    }
  ];
  ...
  const level = calculateLevel(mockUser.xp);
  const progress = calculateProgress(mockUser.xp);
  ...
  const stats = [
    ...
    {
      icon: flame,
      value: mockStreak.current,
      label: 'Série',
      color: 'text-orange-500'
    },
    {
      icon: book,
      value: mockUser.completedLessons.length,
      label: 'Recettes',
      color: 'text-blue-500'
    },
    {
      icon: star,
      value: mockAchievements.filter(a => a.unlocked).length,
      label: 'Succès',
      color: 'text-purple-500'
    }
  ];
        ...
        <div className="mb-6">
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Niveau {level} • {mockUser.xp} XP</p>
            </div>
          </div>
